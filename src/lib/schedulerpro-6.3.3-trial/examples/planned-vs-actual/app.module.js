...
class MyScheduler extends SchedulerPro {
    ...
    get resourceTimeRangeColor() {
        return this.widgetMap.plannedToggle.checked ? this.actualColorCls : this.plannedColorCls;
    }

    get eventColor() {
        return this.widgetMap.plannedToggle.checked ? this.plannedColor : this.actualColor;
    }
    ...
    static configurable = {
        ...
        listeners : {
            ...
            beforeTaskEdit({ source, taskEdit }) {
                ...
                startDateField.label = source.showPlanned ? 'Planned start' : 'Actual start';
                endDateField.label   = source.showPlanned ? 'Planned finish' : 'Actual finish';

                newStartField.label = source.showPlanned ? 'Actual start' : 'Planned start';
                newStartField.name  = source.showPlanned ? 'actualStartDate' : 'plannedStartDate';
                newEndField.label   = source.showPlanned ? 'Actual finish' : 'Planned finish';
                newEndField.name    = source.showPlanned ? 'actualEndDate' : 'plannedEndDate';
            }
        },
    ...
    isValidateDate({ source, oldValue }) {
        const {
            newStartField,
            newEndField
        } = myScheduler.taskEdit.editor.widgetMap;
