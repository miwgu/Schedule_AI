...
class MapPanel extends Panel {
    ...
    setMapStyle() {
        const isDark = DomHelper.themeInfo.name.toLowerCase().includes('dark'),
    ...
    addEventMarker(eventRecord) {
        ...
        if (lat && lon && (!eventRecord.isScheduled || this.timeAxis.isTimeSpanInAxis(eventRecord))) {
            ...
            const color = eventRecord.eventColor || ((_eventRecord$resource = eventRecord.resource) === null || _eventRecord$resource === undefined ? undefined : _eventRecord$resource.eventColor) || '#f0f0f0',
    ...
    showTooltip(eventRecord, centerAtMarker) {
        ...
        <span class="resource"><i class="b-fa b-fa-fw b-fa-user"></i>${((_eventRecord$resource2 = eventRecord.resource) === null || _eventRecord$resource2 === undefined ? undefined : _eventRecord$resource2.name) || 'Unassigned'}</span>
...
class UnplannedGrid extends Grid {
    ...
    static configurable = {
        ...
        columns : [{
            ...
            renderer   : ({
                value
            }) => `<i class="b-fa b-fa-fw b-fa-grip"></i>${StringHelper.encodeHtml(value) || ''}`
        }, {
            text     : 'Location',
