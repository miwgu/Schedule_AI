...
const
    ...
    resetDelay = (args.resetDelay || 5 * 60) * 1000,
    debug      = args.debug;
        ...
        ws.on('message', msg => {
            ...
            try {
                ...
                switch (data.command) {
                    ...
                    // Client updated an event record, broadcast the change and update our dataset
                    case 'updateEvent': {
                        // Find the local "row"
                        const event = dataset.events.find(event => event.id === data.id);

                        if (event) {
                            // Apply all changes to it
                            Object.keys(data.changes).forEach(key => {
                                event[key] = data.changes[key];
                            });
                        }
                        break;
                    }

                    // Client removed event record, broadcast the change and update our dataset
                    case 'removeEvent':
                        // Remove the local "rows"
                        dataset.events = dataset.events.filter(event => data.records.find(r => r.id !== event.id));
                        break;

                    // Client added a event record, broadcast the change and update our dataset
                    case 'addEvent':
                        // Add new event
                        dataset.events.push(...data.records);
                        break;
                }
...
function readDataSet() {
    try {
        debugLog('Reading dataset');
        dataset = JSON.parse(fs.readFileSync('data/data.json'));
    }
    catch (error) {
        logError(error);
    }
...
function resetDataSet(userName) {
    ...
    readDataSet();
    broadcast(null, { command : 'dataset', dataset });
