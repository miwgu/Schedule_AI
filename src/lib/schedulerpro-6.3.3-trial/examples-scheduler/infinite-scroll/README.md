...
This demo has two backends, one in PHP, and one built with Express.js. To be able to run this demo locally, you need to
start one of them.
...
To start the server, navigate to the infinite-scroll/backend folder and run:
...
In `app.js`, make sure the store's URLs are correct. They should match the port your server is running.
