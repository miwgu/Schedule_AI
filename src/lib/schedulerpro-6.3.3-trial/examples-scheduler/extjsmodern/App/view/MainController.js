...
Ext.define('App.view.MainController', {
    ...
    init : function() {
        ...
        schedulerPanel.on({
            beforeeventedit : function(sender, event) {
                me.editEvent(event.eventRecord);
    ...
    addTask : function(startDate, resourceRecord) {
        const editor       = this.getEventEditor(),
    ...
    addTimeRange : function() {
        const editor         = this.getTimeRangeEditor(),
            schedulerPanel = this.lookup('schedulerPanel'),
    ...
    saveTask : function() {
        ...
        if (editorForm.isValid()) {
            ...
            if (editor.mode === 'create') {
                editorForm.validate();
                if (editorForm.isValid()) {
                    !editor.getEvent()?.isCreating && schedulerPanel.getEventStore().add(values);
                    editor.hide();
                }
            }
    ...
    getTimeRangeEditor : function() {
        let editor = this.timeRangeEditor;
