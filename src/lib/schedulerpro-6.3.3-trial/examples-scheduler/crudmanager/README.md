# Scheduler CrudManager Demo

This is a demo showing our Bryntum Scheduler component running with a PHP backend.
...
The first step is to configure MySQL DB access parameters. Rename (or copy) `php/config.template.php` file into
`php/config.php` and edit its content. Change `DBHOST`, `DBUSERNAME`, `DBUSERPASSWORD` and `DBNAME` to proper values.

For example in the following code we define that username is `root` and password is `password` and database named
`scheduler` is located at `localhost` host:

```php
const DBHOST = 'localhost';
const DBUSERNAME = 'root';
const DBUSERPASSWORD = 'password';
const DBNAME = 'schedulerpro';
```

# Create sample database

Create sample database with one of the following ways:

### Using mysql cmdline

```shell
mysql --host=localhost --user=root --password=password --database=scheduler < path_to_demo_location/sql/setup.sql"
```

### Using setup.php script

Open http://localhost/demo_location_url/php/setup.php in browser. This will create `scheduler` database for you.

### Using SQL manager tool

Import database from `sql/setup.sql` script with SQL manager of your choice like MySQL Workbench or phpMyAdmin.

# Run the demo

Open http://localhost/demo_location_url/index.html in browser.
